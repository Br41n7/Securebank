...
class User(AbstractUser):
    ...
    account_locked_until = models.DateTimeField(
        _("account locked until"), null=True, blank=True
    )
...
class UserProfile(models.Model):
    ...
    id_type = models.CharField(
        _("ID type"), max_length=20, choices=ID_TYPE_CHOICES, blank=True
    )
    id_number = models.CharField(_("ID number"), max_length=50, blank=True)
    id_document = models.ImageField(
        _("ID document"), upload_to="kyc_documents/", null=True, blank=True
    )
    proof_of_address = models.ImageField(
        _("proof of address"), upload_to="address_documents/", null=True, blank=True
    )
    kyc_status = models.CharField(
        _("KYC status"), max_length=20, choices=KYC_STATUS_CHOICES, default="PENDING"
    )
...
class BankAccount(models.Model):
    ...
    balance = models.DecimalField(
        _("balance"), max_digits=15, decimal_places=2, default=Decimal("0.00")
    )
    available_balance = models.DecimalField(
        _("available balance"), max_digits=15, decimal_places=2, default=Decimal("0.00")
    )
    frozen_balance = models.DecimalField(
        _("frozen balance"), max_digits=15, decimal_places=2, default=Decimal("0.00")
    )
    currency = models.CharField(_("currency"), max_length=3, default="NGN")
    status = models.CharField(
        _("status"), max_length=20, choices=ACCOUNT_STATUS_CHOICES, default="ACTIVE"
    )
    is_default = models.BooleanField(_("default account"), default=False)
    daily_limit = models.DecimalField(
        _("daily limit"), max_digits=15, decimal_places=2, default=Decimal("500000.00")
    )
    ...
    def can_withdraw(self, amount):
        ...
        return (
            self.is_active
            and amount <= self.available_balance
            and amount <= self.single_transaction_limit
        )
...
class SecuritySettings(models.Model):
    ...
    transaction_notifications = models.BooleanField(
        _("transaction notifications"), default=True
    )
    ...
    require_otp_for_transactions = models.BooleanField(
        _("require OTP for transactions"), default=True
    )
    transaction_threshold = models.DecimalField(
        _("transaction threshold"),
        max_digits=15,
        decimal_places=2,
    ...
    auto_logout_time = models.PositiveIntegerField(
        _("auto logout time (minutes)"), default=60
    )
    concurrent_sessions = models.BooleanField(
        _("allow concurrent sessions"), default=False
    )
