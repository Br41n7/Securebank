...
class PasswordComplexityValidator:
    ...
    def validate(self, password, user=None):
        ...
        if re.search(r"(.)\1{2,}", password):  # Three or more repeated characters
            errors.append(
                _("Password cannot contain three or more repeated characters in a row.")
            )
    ...
    def __call__(self, value):
        ...
        if len(value) != self.length:
            raise ValidationError(
                _("Account number must be exactly {length} digits long.").format(
                    length=self.length
                )
            )
        ...
        if len(set(value)) < 3:
            raise ValidationError(
                _("Account number is too simple. Please use a different number.")
            )
...
class PhoneNumberValidator:
    ...
    def __call__(self, value):
        ...
        if not (value.startswith("+") or value.startswith("0")):
            raise ValidationError(
                _(
                    "Phone number must start with '+' for international format or '0' for local format."
                )
            )
...
class AmountValidator:
    ...
    def __call__(self, value):
        ...
        if value < self.min_amount:
            raise ValidationError(
                _("Minimum amount is {min_amount}.").format(min_amount=self.min_amount)
            )

        if self.max_amount and value > self.max_amount:
            raise ValidationError(
                _("Maximum amount is {max_amount}.").format(max_amount=self.max_amount)
            )
