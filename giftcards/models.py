...
class GiftCardType(models.Model):
    ...
    logo = models.ImageField(_('logo'), upload_to='giftcard_logos/', null=True, blank=True)
    is_active = models.BooleanField(_('active'), default=True)
    
    # Rates and limits
    buy_rate = models.DecimalField(_('buy rate %'), max_digits=5, decimal_places=2, default=Decimal('85.00'))  # Percentage of face value
    sell_rate = models.DecimalField(_('sell rate %'), max_digits=5, decimal_places=2, default=Decimal('95.00'))  # Percentage of face value
    min_amount = models.DecimalField(_('minimum amount'), max_digits=10, decimal_places=2, default=Decimal('10.00'))
    max_amount = models.DecimalField(_('maximum amount'), max_digits=10, decimal_places=2, default=Decimal('1000.00'))
    
    # Supported countries
    supported_countries = models.TextField(_('supported countries'), help_text="Comma-separated country codes", default="US,CA,UK")
...
class GiftCard(models.Model):
    ...
    seller = models.ForeignKey(User, on_delete=models.CASCADE, related_name='sold_giftcards')
    gift_card_type = models.ForeignKey(GiftCardType, on_delete=models.CASCADE, related_name='cards')
    ...
    # Receipt/Proof
    receipt_image = models.ImageField(_('receipt image'), upload_to='giftcard_receipts/', null=True, blank=True)
    card_image = models.ImageField(_('card image'), upload_to='giftcard_images/', null=True, blank=True)
    
    # Status and pricing
    status = models.CharField(_('status'), max_length=20, choices=CARD_STATUS_CHOICES, default='PENDING')
...
class GiftCardTransaction(models.Model):
    ...
    # Parties involved
    buyer = models.ForeignKey(User, on_delete=models.CASCADE, related_name='bought_giftcards', null=True, blank=True)
    seller = models.ForeignKey(User, on_delete=models.CASCADE, related_name='sold_giftcard_transactions')
...
class GiftCardRate(models.Model):
    ...
    """
    gift_card_type = models.ForeignKey(GiftCardType, on_delete=models.CASCADE, related_name='rates')
...
class GiftCardDispute(models.Model):
    ...
    dispute_type = models.CharField(_('dispute type'), max_length=20, choices=DISPUTE_TYPE_CHOICES)
    description = models.TextField(_('description'))
    evidence = models.ImageField(_('evidence'), upload_to='dispute_evidence/', null=True, blank=True)
    
    status = models.CharField(_('status'), max_length=20, choices=DISPUTE_STATUS_CHOICES, default='OPEN')
...
class GiftCardInventory(models.Model):
    ...
    """
    gift_card_type = models.ForeignKey(GiftCardType, on_delete=models.CASCADE, related_name='inventory')
    ...
    def __str__(self):
        return f"{self.gift_card_type.name} ${self.face_value} - {self.quantity} available"