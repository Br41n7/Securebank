...
class ServiceProvider(models.Model):
    ...
    logo = models.ImageField(
        _("logo"), upload_to="provider_logos/", null=True, blank=True
    )
    ...
    service_charge = models.DecimalField(
        _("service charge"), max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
...
class AirtimeTopUp(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="airtime_topups"
    )
    account = models.ForeignKey(
        BankAccount, on_delete=models.CASCADE, related_name="airtime_topups"
    )
    provider = models.ForeignKey(
        ServiceProvider, on_delete=models.CASCADE, related_name="airtime_topups"
    )
    ...
    provider_reference = models.CharField(
        _("provider reference"), max_length=100, blank=True
    )
...
class BillPayment(models.Model):
    ...
    reference = models.CharField(_("transaction reference"), max_length=50, unique=True)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="bill_payments"
    )
    account = models.ForeignKey(
        BankAccount, on_delete=models.CASCADE, related_name="bill_payments"
    )
    provider = models.ForeignKey(
        ServiceProvider, on_delete=models.CASCADE, related_name="bill_payments"
    )

    # Bill details
    bill_type = models.CharField(
        _("bill type"), max_length=20, choices=BILL_TYPE_CHOICES
    )
    ...
    token = models.CharField(
        _("token"), max_length=200, blank=True
...
class SchoolFeePayment(models.Model):
    ...
    reference = models.CharField(_("transaction reference"), max_length=50, unique=True)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="school_fee_payments"
    )
    account = models.ForeignKey(
        BankAccount, on_delete=models.CASCADE, related_name="school_fee_payments"
    )
...
class ServiceTransaction(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="service_transactions"
    )
...
class SavedService(models.Model):
    ...
    customer_details = models.JSONField(
        _("customer details"), default=dict
    )  # Store customer ID, meter number, etc.
